  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const statsRes = await fetch(`http://localhost:3001/api/chartdata/stats?${params.toString()}`);
        const statsBody = statsRes.ok ? await statsRes.json() : null;

        const prodRes = await fetch(`http://localhost:3001/api/chartdata/produtos?${params.toString()}`);
