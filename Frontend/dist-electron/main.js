...
function coerceData$1(it, types2, coerceTo) {
  const { gen, data, opts } = it;
  const dataType2 = gen.let("dataType", (0, codegen_1$T._)`typeof ${data}`);
  const coerced = gen.let("coerced", (0, codegen_1$T._)`undefined`);
  if (opts.coerceTypes === "array") {
    gen.if((0, codegen_1$T._)`${dataType2} == 'object' && Array.isArray(${data}) && ${data}.length == 1`, () => gen.assign(data, (0, codegen_1$T._)`${data}[0]`).assign(dataType2, (0, codegen_1$T._)`typeof ${data}`).if(checkDataTypes$1(types2, data, opts.strictNumbers), () => gen.assign(coerced, data)));
  }
  gen.if((0, codegen_1$T._)`${coerced} !== undefined`);
  ...
  function coerceSpecificType(t2) {
    switch (t2) {
      case "string":
        gen.elseIf((0, codegen_1$T._)`${dataType2} == "number" || ${dataType2} == "boolean"`).assign(coerced, (0, codegen_1$T._)`"" + ${data}`).elseIf((0, codegen_1$T._)`${data} === null`).assign(coerced, (0, codegen_1$T._)`""`);
        return;
      case "number":
        gen.elseIf((0, codegen_1$T._)`${dataType2} == "boolean" || ${data} === null
              || (${dataType2} == "string" && ${data} && ${data} == +${data})`).assign(coerced, (0, codegen_1$T._)`+${data}`);
        return;
      case "integer":
        gen.elseIf((0, codegen_1$T._)`${dataType2} === "boolean" || ${data} === null
              || (${dataType2} === "string" && ${data} && ${data} == +${data} && !(${data} % 1))`).assign(coerced, (0, codegen_1$T._)`+${data}`);
        return;
      case "boolean":
        gen.elseIf((0, codegen_1$T._)`${data} === "false" || ${data} === 0 || ${data} === null`).assign(coerced, false).elseIf((0, codegen_1$T._)`${data} === "true" || ${data} === 1`).assign(coerced, true);
        return;
      case "null":
        gen.elseIf((0, codegen_1$T._)`${data} === "" || ${data} === 0 || ${data} === false`);
        gen.assign(coerced, null);
        return;
      case "array":
        gen.elseIf((0, codegen_1$T._)`${dataType2} === "string" || ${dataType2} === "number"
              || ${dataType2} === "boolean" || ${data} === null`).assign(coerced, (0, codegen_1$T._)`[${data}]`);
    }
  }
}
function assignParentData$1({ gen, parentData, parentDataProperty }, expr) {
  gen.if((0, codegen_1$T._)`${parentData} !== undefined`, () => gen.assign((0, codegen_1$T._)`${parentData}[${parentDataProperty}]`, expr));
}
function checkDataType$1(dataType2, data, strictNums, correct = DataType$1.Correct) {
  const EQ = correct === DataType$1.Correct ? codegen_1$T.operators.EQ : codegen_1$T.operators.NEQ;
  let cond;
  switch (dataType2) {
    case "null":
      return (0, codegen_1$T._)`${data} ${EQ} null`;
    case "array":
      cond = (0, codegen_1$T._)`Array.isArray(${data})`;
      break;
    case "object":
      cond = (0, codegen_1$T._)`${data} && typeof ${data} == "object" && !Array.isArray(${data})`;
      break;
    case "integer":
      cond = numCond((0, codegen_1$T._)`!(${data} % 1) && !isNaN(${data})`);
      break;
    case "number":
      cond = numCond();
      break;
    default:
      return (0, codegen_1$T._)`typeof ${data} ${EQ} ${dataType2}`;
  }
  return correct === DataType$1.Correct ? cond : (0, codegen_1$T.not)(cond);
  function numCond(_cond = codegen_1$T.nil) {
    return (0, codegen_1$T.and)((0, codegen_1$T._)`typeof ${data} == "number"`, _cond, strictNums ? (0, codegen_1$T._)`isFinite(${data})` : codegen_1$T.nil);
  }
}
...
function checkDataTypes$1(dataTypes, data, strictNums, correct) {
  ...
  if (types2.array && types2.object) {
    const notObj = (0, codegen_1$T._)`typeof ${data} != "object"`;
    cond = types2.null ? notObj : (0, codegen_1$T._)`!${data} || ${notObj}`;
    delete types2.null;
    delete types2.array;
    delete types2.object;
  } else {
    cond = codegen_1$T.nil;
  }
  if (types2.number)
    delete types2.integer;
  for (const t2 in types2)
    cond = (0, codegen_1$T.and)(cond, checkDataType$1(t2, data, strictNums, correct));
  return cond;
}
dataType$1.checkDataTypes = checkDataTypes$1;
const typeError$1 = {
  message: ({ schema }) => `must be ${schema}`,
  params: ({ schema, schemaValue }) => typeof schema == "string" ? (0, codegen_1$T._)`{type: ${schema}}` : (0, codegen_1$T._)`{type: ${schemaValue}}`
};
...
const boolSchema_1$1 = requireBoolSchema$1();
const dataType_1$3 = dataType$1;
const applicability_1$2 = applicability$1;
const dataType_2$1 = dataType$1;
const defaults_1$1 = requireDefaults$1();
const keyword_1$1 = requireKeyword$1();
const subschema_1$1 = requireSubschema$1();
...
function validateFunction$1({ gen, validateName, schema, schemaEnv, opts }, body) {
  if (opts.code.es5) {
    gen.func(validateName, (0, codegen_1$S._)`${names_1$b.default.data}, ${names_1$b.default.valCxt}`, schemaEnv.$async, () => {
      gen.code((0, codegen_1$S._)`"use strict"; ${funcSourceUrl$1(schema, opts)}`);
      destructureValCxtES5$1(gen, opts);
      gen.code(body);
    });
  } else {
    gen.func(validateName, (0, codegen_1$S._)`${names_1$b.default.data}, ${destructureValCxt$1(opts)}`, schemaEnv.$async, () => gen.code(funcSourceUrl$1(schema, opts)).code(body));
  }
}
function destructureValCxt$1(opts) {
  return (0, codegen_1$S._)`{${names_1$b.default.instancePath}="", ${names_1$b.default.parentData}, ${names_1$b.default.parentDataProperty}, ${names_1$b.default.rootData}=${names_1$b.default.data}${opts.dynamicRef ? (0, codegen_1$S._)`, ${names_1$b.default.dynamicAnchors}={}` : codegen_1$S.nil}}={}`;
}
function destructureValCxtES5$1(gen, opts) {
  gen.if(names_1$b.default.valCxt, () => {
    gen.var(names_1$b.default.instancePath, (0, codegen_1$S._)`${names_1$b.default.valCxt}.${names_1$b.default.instancePath}`);
    gen.var(names_1$b.default.parentData, (0, codegen_1$S._)`${names_1$b.default.valCxt}.${names_1$b.default.parentData}`);
    gen.var(names_1$b.default.parentDataProperty, (0, codegen_1$S._)`${names_1$b.default.valCxt}.${names_1$b.default.parentDataProperty}`);
    gen.var(names_1$b.default.rootData, (0, codegen_1$S._)`${names_1$b.default.valCxt}.${names_1$b.default.rootData}`);
    if (opts.dynamicRef)
      gen.var(names_1$b.default.dynamicAnchors, (0, codegen_1$S._)`${names_1$b.default.valCxt}.${names_1$b.default.dynamicAnchors}`);
  }, () => {
    gen.var(names_1$b.default.instancePath, (0, codegen_1$S._)`""`);
    gen.var(names_1$b.default.parentData, (0, codegen_1$S._)`undefined`);
    gen.var(names_1$b.default.parentDataProperty, (0, codegen_1$S._)`undefined`);
    gen.var(names_1$b.default.rootData, names_1$b.default.data);
    if (opts.dynamicRef)
      gen.var(names_1$b.default.dynamicAnchors, (0, codegen_1$S._)`{}`);
  });
}
...
function subSchemaObjCode$1(it, valid2) {
  ...
  gen.var(valid2, (0, codegen_1$S._)`${errsCount} === ${names_1$b.default.errors}`);
}
...
function commentKeyword$1({ gen, schemaEnv, schema, errSchemaPath, opts }) {
  ...
  if (opts.$comment === true) {
    gen.code((0, codegen_1$S._)`${names_1$b.default.self}.logger.log(${msg})`);
  } else if (typeof opts.$comment == "function") {
    const schemaPath = (0, codegen_1$S.str)`${errSchemaPath}/$comment`;
    const rootName = gen.scopeValue("root", { ref: schemaEnv.root });
    gen.code((0, codegen_1$S._)`${names_1$b.default.self}.opts.$comment(${msg}, ${schemaPath}, ${rootName}.schema)`);
  }
}
function returnResults$1(it) {
  const { gen, schemaEnv, validateName, ValidationError: ValidationError3, opts } = it;
  if (schemaEnv.$async) {
    gen.if((0, codegen_1$S._)`${names_1$b.default.errors} === 0`, () => gen.return(names_1$b.default.data), () => gen.throw((0, codegen_1$S._)`new ${ValidationError3}(${names_1$b.default.vErrors})`));
  } else {
    gen.assign((0, codegen_1$S._)`${validateName}.errors`, names_1$b.default.vErrors);
    if (opts.unevaluated)
      assignEvaluated$1(it);
    gen.return((0, codegen_1$S._)`${names_1$b.default.errors} === 0`);
  }
}
...
function schemaKeywords$1(it, types2, typeErrors, errsCount) {
  ...
  function groupKeywords(group) {
    ...
    if (!allErrors)
      gen.if((0, codegen_1$S._)`${names_1$b.default.errors} === ${errsCount || 0}`);
  }
}
  ...
  invalid$data() {
    ...
    function wrong$DataType() {
      if (schemaType.length) {
        ...
        return (0, codegen_1$S._)`${(0, dataType_2$1.checkDataTypes)(st, schemaCode, it.opts.strictNumbers, dataType_2$1.DataType.Wrong)}`;
      }
...
function getData$1($data, { dataLevel, dataNames, dataPathArr }) {
  ...
  for (const segment of segments) {
    if (segment) {
      data = (0, codegen_1$S._)`${data}${(0, codegen_1$S.getProperty)((0, util_1$O.unescapeJsonPointer)(segment))}`;
...
function callRef$1(cxt, v, sch, $async) {
  ...
  const passCxt = opts.passContext ? names_1$9.default.this : codegen_1$Q.nil;
  if ($async)
    callAsyncRef();
  else
    callSyncRef();
  ...
  function addErrorsFrom(source) {
    ...
    gen.assign(names_1$9.default.vErrors, (0, codegen_1$Q._)`${names_1$9.default.vErrors} === null ? ${errs} : ${names_1$9.default.vErrors}.concat(${errs})`);
    gen.assign(names_1$9.default.errors, (0, codegen_1$Q._)`${names_1$9.default.vErrors}.length`);
  }
  function addEvaluatedFrom(source) {
    ...
    if (it.props !== true) {
      if (schEvaluated && !schEvaluated.dynamicProps) {
        ...
        it.props = util_1$M.mergeEvaluated.props(gen, props, it.props, codegen_1$Q.Name);
      }
    }
    if (it.items !== true) {
      if (schEvaluated && !schEvaluated.dynamicItems) {
        ...
        it.items = util_1$M.mergeEvaluated.items(gen, items2, it.items, codegen_1$Q.Name);
      }
    }
  }
}
...
const def$_ = {
  ...
  code(cxt) {
    ...
    const len = it.opts.unicode === false ? (0, codegen_1$N._)`${data}.length` : (0, codegen_1$N._)`${(0, util_1$L.useFunc)(cxt.gen, ucs2length_1$1.default)}(${data})`;
...
const def$Z = {
  ...
  code(cxt) {
    ...
    const regExp = $data ? (0, codegen_1$M._)`(new RegExp(${schemaCode}, ${u}))` : (0, code_1$h.usePattern)(cxt, schema);
...
const def$M = {
  ...
  code(cxt) {
    ...
    cxt.ok((0, codegen_1$A._)`${errsCount} === ${names_1$8.default.errors}`);
...
function dynamicAnchor(cxt, anchor) {
  ...
  const v = (0, codegen_1$w._)`${names_1$7.default.dynamicAnchors}${(0, codegen_1$w.getProperty)(anchor)}`;
...
function dynamicRef(cxt, ref2) {
  ...
  function _dynamicRef(valid2) {
    if (it.schemaEnv.root.dynamicAnchors[anchor]) {
      const v = gen.let("_v", (0, codegen_1$v._)`${names_1$6.default.dynamicAnchors}${(0, codegen_1$v.getProperty)(anchor)}`);
...
const def$w = {
  ...
  code(cxt) {
    ...
    it.props = true;
    cxt.ok((0, codegen_1$u._)`${errsCount} === ${names_1$5.default.errors}`);
...
const rules_1 = rules;
const applicability_1$1 = requireApplicability();
