...
>>>>>>> 0c65bce1464ec86aa881b1f85eb9d9245190f90c
...
function returnResults(it) {
  const { gen, schemaEnv, validateName, ValidationError: ValidationError3, opts } = it;
  if (schemaEnv.$async) {
    gen.if((0, codegen_1$R._)`${names_1$a.default.errors} === 0`, () => gen.return(names_1$a.default.data), () => gen.throw((0, codegen_1$R._)`new ${ValidationError3}(${names_1$a.default.vErrors})`));
  } else {
...
ipcMain$1.handle("start-fork", async (_event, { script, args = [] } = {}) => {
  ...
  if (path.isAbsolute(script)) {
    ...
    const possiblePaths = [
      ...
      // From electron build dir
      path.join(process.env.APP_ROOT, script),
      // From app root if defined
      path.join(path.dirname(__dirname), script),
      // From Frontend dir
      path.join(path.dirname(path.dirname(__dirname)), script),
      // From parent of Frontend (project root)
      path.resolve(script)
