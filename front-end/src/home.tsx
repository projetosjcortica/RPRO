...
// Nova função para agregar produtos (colunas dos values)
function aggregateProducts(rows: Entry[]): { productData: ChartDatum[]; productSums: Record<string, number>; totalProducts: number } {
  ...
  for (const r of valid) {
    r.values.forEach((value, index) => {
      const productKey = `Prod_${index + 1}`
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
              {periods.map((p) => {
                ...
                return (
                  <div key={p.key} className="border rounded p-3 flex flex-col items-center">
                    <div className="mb-2 text-sm font-medium">{p.label}</div>
                    <div className="w-32 h-32">
                      <ChartContainer config={chartConfig} className="w-full h-full">
                        <PieChart>
                          <ChartTooltip content={<ChartTooltipContent hideLabel />} formatter={(v: number) => [`${v} KG`, null]} />
                          <Pie data={allProductsForChart.map(d => ({ browser: d.name, visitors: d.value }))} dataKey="visitors" nameKey="browser" outerRadius={60} innerRadius={24} labelLine={false}>
                            {allProductsForChart.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                          </Pie>
                        </PieChart>
                      </ChartContainer>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 scrollbar-custom scrollbar-thin">
              {periods.map((p) => {
                const agg = aggregateForRange(p.key)
                ...
                return (
                  <div key={p.key} className="border rounded p-3">
                    ...
                    <div className="space-y-1 mb-2 max-h-96 overflow-y-auto">
                      {allProducts.map((t, i) => (
                        <div key={i} className="flex items-center justify-between p-1 bg-gray-50 rounded text-xs">
                          ...
                          <div className="font-semibold">{t.value} KG</div>
                        </div>
                      ))}
                    ...
                    <div className="text-xs text-gray-600 border-t pt-1">
                      <div>Total Produtos: {agg.productTotal} KG | Batidas: {batidas}</div>
                    </div>
                  </div>
                )
              })}
